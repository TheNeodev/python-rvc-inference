...
def validate_config_and_files():
    ...
    missing_configs = {file: env for file, env in required_files.items() if env is None and not (BASE_DIR / file).exists()}
    ...
    if missing_configs:
        ...
        raise RuntimeError("Ensure all model paths are correctly set in environment variables or files exist in the directory.")
    else:
